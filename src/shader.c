...
/*   Updated: 2020/06/18 14:51:53 by grolash          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	program_link_error(GLuint shader_program)
{
	...
	if (!success)
	{
		...
		ft_putstr_fd(2, "ERROR::SADER::PROGRAM::LINKING_PAILED:\n");
...
int		shader_link()
{
	...
	if(shader_compil(&vertex_shader) ||
			frag_shader_compil(&fragment_shader))
		return (-1);
